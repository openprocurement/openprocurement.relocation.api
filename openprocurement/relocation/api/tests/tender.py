...
class OpenUACompatitiveDialogueStage2TenderOwnershipChangeTest(CompatitiveDialogueOwnershipWebTest, TenderOwnershipChangeTest):
    ...
    def test_change_tender_ownership(self):
        ...
        tender1_id = self.tender_id
        response = self.app.post_json('/tenders/{}/ownership'.format(tender1_id), {"data": {"id": 12}}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.json['errors'], [
            {u'description': u'This field is required.', u'location': u'body', u'name': u'transfer'}
        ])
        ...
        response = self.app.get('/tenders/{}'.format(tender1_id))
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.json['data']['owner'], self.first_owner)

        # geting tender transfer token
        response = self.app.patch_json('/tenders/{}/credentials?acc_token={}'.format(tender1_id, test_access_token_stage1), {'data': ''})
        ...
        response = self.app.post_json('/tenders/{}/ownership'.format(tender1_id),
        ...
        transfer = response.json['data']
        transfer_modification_date = transfer['date']
        self.assertEqual(transfer['usedFor'], '/tenders/' + tender1_id)
        self.assertNotEqual(transfer_creation_date, transfer_modification_date)

        # second owner can change the tender
        response = self.app.patch_json('/tenders/{}?acc_token={}'.format(tender1_id, new_access_token),
                                       {"data": {"tenderPeriod": {"endDate": "2017-03-04T02:41:41.042506+03:00"}}})
        self.assertEqual(response.status, '200 OK')
        self.assertNotIn('transfer', response.json['data'])
        self.assertNotIn('transfer_token', response.json['data'])
        self.assertIn('owner', response.json['data'])
        self.assertEqual(response.json['data']['owner'], self.second_owner)
        self.assertEqual(response.json['data']["tenderPeriod"]['endDate'], "2017-03-04T02:41:41.042506+03:00")
        ...
        tender2_id = tender2['id']
        self.tender_id = tender2_id
        self.set_status('draft.stage2')

        self.app.authorization = ('Basic', (self.first_owner, ''))
        response = self.app.patch_json('/tenders/{}/credentials?acc_token={}'.format(tender2_id, test_access_token_stage1), {'data': ''})
        tender2_transfer_token = response.json['access']['transfer']
        self.app.authorization = ('Basic', (self.second_owner, ''))
        response = self.app.post_json('/tenders/{}/ownership'.format(tender2['id']),
        ...
        # but tender is not stored with new credentials)
        transfer_doc = self.db.get(transfer['id'])
        transfer_doc['usedFor'] = '/tenders/' + tender2['id']
        self.db.save(transfer_doc)
        response = self.app.post_json('/tenders/{}/ownership'.format(tender2['id']),
                                      {"data": {"id": transfer['id'], 'transfer': tender2_transfer_token}}, status=200)
        self.assertEqual(self.second_owner, response.json['data']['owner'])
        ...
        # set test mode and try to change ownership
        self.app.authorization = ('Basic', ('administrator', ''))
        response = self.app.patch_json('/tenders/{}'.format(tender1_id), {'data': {'mode': 'test'}})
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.json['data']['mode'], 'test')
